...
```
```{r}
# remove old file so next step reloads from disk
...
# specify and fit structural equation model across imputations
# includes autoregressive, mediation, and direct paths with WLSMV
# pooled fit indices and parameter estimates are displayed
...
# simplified SEM used to inspect residual covariances
model_quick <- '
  # structural
 trust_w2_c  ~ d1 * trust_w1_c
 trust_w3_c  ~ d2 * trust_w2_c
...
Example text and plots removed from final analysis. Retained here as a comment
for reference on basic R Markdown features.
